$.urlParam = function (name) {
  ...
  $(document).ready(function () {
    ...
    console.log(url);
    $.ajax({
      url: "http://15.164.188.135:8080/app/production/list/asc/" + url,
      type: "GET",
      ...
      success: function (result) {
        ...
        for(var i = 0; i < result.length; i++){
          /*'<img class="pro-img" src="http://15.164.188.135:8080/resources/'+data[i].uploadPath+'/s_'+data[i].uuid+'_'+data[i].fileName+' " width="90px;" height="85px;"/>'*/
          var tag = 
          '<div class="pro2"> <div class="imgess"><img  class="pro-img" src="http://15.164.188.135:8080/resources/'+result[i].uploadPath+'/s_'+result[i].uuid+'_'+result[i].fileName+' " width="90px;" height="85px;"></div>'+
          '<div class="img-name" >'+
              '<p class="pro-title">'+
              result[i].title+
              '</p>'+
