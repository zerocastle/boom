...
@AllArgsConstructor     //모든 필드(전역변수)를 파라미터로 가지는 생성자를 만든다. 간단하게 말해서 여기서 호출하는 클래스에 대한 생성자(Service)를 자동으로 만든다
@RequestMapping(value = "/app/**")	// /app/**로 오는 데이터 접근은 여기서 처리한다.
public class AppController { //메인 클래스
	
	private static final Logger logger = LoggerFactory.getLogger(AppController.class); //콘솔 찍어주는 것     지속적으로 logger로 상황보고 해주는 것
...
//	       System.out.println("mlogin");
	    ...
	    logger.info("닉네임 : sdsdddddddddddddddddddddddddddddddddddddddddddd" + map); //찍어주는것(확인)
	       ...
	       MemberVO member = service.nickNameCheck(name);//실행순서 1.Controller 2.service 3.DAO 4.memberMapper.xml(SqlSession/mybatis)
	       ...
	       logger.info("닉네임 : sdsdddddddddddddddddddddddddddddddddddddddddddd" + member);//찍어주는것(확인)
			/* System.out.println(member.getNickname() + member.getM_password()) */;
			//m_num=3, nickname=jack, phone=123456, m_password=jack, email=i__pop@naver.com, manner=0, 
			//intro=null, warning=0, img_path=null, partner_signal=1, acc_num=789, acc_name=부산
			if(member == null) {
				map.put("signal", "fail");
			}else {
...
//		}
		...
		@ResponseBody
		public Map<String, String> nickNameCheck(Model model,  @RequestBody Map<String , String> map)
				throws Exception {
			MemberVO member = new MemberVO();
			try {
				logger.info("닉네임 체크 : " + map);
				member = service.nickNameCheck(map.get("nickname"));
				logger.info(member.getNickname()+"nnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnn");
